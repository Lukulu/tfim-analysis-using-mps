...
'''calculate E0 (ground state energy) depending on the
   maximal bond dimension Dmax for N particles for different J/beta'''
...
for i in range(1, D, 1):
    conf = ImagiTime_config(N=N, J=J, beta=beta, dt0=dt0, eps_trunc=eps_trunc, Dmax=i, eps_dt=eps_dt, dt_red=dt_red, eps_tol=eps_tol, phys_gap=phys_gap, bdim_start=bdim_start)
    time_stamp = datetime.now().strftime("%y%m%d%H%M%S")
    E0, E, groundstate = getGroundstate(conf, time_stamp)
    print("val %f %30.25f" % (i, E))
