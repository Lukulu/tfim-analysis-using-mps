...
'''calculate coefficient c of the ground state depending
...
#calculate coeffcient c from a given MPS mps
def calc_coeff_state(mps):
    N = len(mps)
    n1 = 2**N-1
    n2 = 2**N-4
    ind1 = getBinaryRep(n1, N)
    ind2 = getBinaryRep(n2, N)
    prod1 = mps[0][:,:,ind1[N-1]]
    prod2 = mps[0][:,:,ind2[N-1]]
    for j in range(1,N):
        prod1 = torch.matmul(prod1, mps[j][:,:,ind1[(N-1)-j]])
        prod2 = torch.matmul(prod2, mps[j][:,:,ind2[(N-1)-j]])
    p1 = prod1.numpy()
    p2 = prod2.numpy()
    return(np.real(p2/p1))

#parameter
beta = 10**np.linspace(-2, 1, 20)
J = np.full(len(beta), 1)
...
for i in range(len(J)):
    ...
    c = calc_coeff_state(groundstate)
    x = J[i]/beta[i]

    #print results
    print("val " + str(x) + " " + str(c[0].item()))
